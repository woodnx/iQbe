...
export interface CategoryAddModalButtonProps<T extends boolean>
  extends ButtonProps {
  isSub: T;
...
export default function CategoryCreateModalButton<T extends boolean>({
  ...
  const DefaultButton = () => (
    <Button leftSection={<IconPlus />} onClick={modal} {...others}>
      {value}
    </Button>
  );

  const MobileButton = () => (
    <ActionIcon onClick={modal}>
      <IconNewSection />
    </ActionIcon>
  );

  const ShowButton = () => (isMobile ? <MobileButton /> : <DefaultButton />);
