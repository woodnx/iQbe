...
type CategoryBase = components["schemas"]["Category"] &
  components["schemas"]["SubCategory"];
...
export default function FilteringCategoriesBase({
  ...
  const options = data?.map(({ id, name, description }) => (
    <Combobox.Option value={String(id)} key={id} mb={4}>
  ...
  return (
    <Combobox
      store={combobox}
      onOptionSubmit={(v) =>
        handleValueSelect(data?.find((d) => d.id == Number(v)))
      }
      withinPortal={true}
      {...others}
    >
